...
const Course = ({ pageContext }) => {
    ...
    const { slug, title, description, authors, version, sections, tags } = pageContext;

  return <Layout>
    <section>
        <div class="mx-auto max-w-screen-xl px-4 py-16">
            <div class="flex gap-2">
                {tags.map(item => <span class={`rounded-3xl bg-slate-100 px-2 ${GetTagStyle(item)}`}><Link to={`/categories/${item}`}>{item}</Link></span>)}
            </div>
